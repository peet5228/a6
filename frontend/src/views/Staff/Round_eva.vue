<template>
    <v-container fluid class="py-10">
                 <v-card class="pa-4">
                        <h1 class="text-h5 text-center font-weight-bold text-maroon">จัดการรอบการประเมิน</h1>
                <v-form @submit.prevent="saveMember">
                        ...
                        <v-row class="mt-4">
                            ...
                            <v-col cols="12" md="6">                   
                             ...
                             <v-col cols="12" md="6">
                                <v-select v-model="form.status_sys" :items="[{title:'เปิด',value: 'y'},{title:'ปิด',value: 'n'}]" label="รอบการประเมิน" />
                            </v-col>     
                        ...
                        <tbody>
                            <tr v-for="(items,index) in result" :key="items.id_sys">
                                ...
                                <td class="text-center border">
                                    ...
                                    <v-btn color="error" class="text-white" size="small" @click="del(items.id_sys)">ลบ</v-btn>
                                </td>
                            </tr>
...
const fetch = async () => {
    try{
        const res = await axios.get(`${api}/round_sys`)
...
const saveMember = async () =>{
    try{
        if(form.value.id_sys){
            await axios.put(`${api}/round_sys/${form.value.id_sys}`,form.value)
        }else{
            await axios.post(`${api}/round_sys`,form.value)
        }
...
const del = async (id_sys:number) => {
    try{
        ...
        await axios.delete(`${api}/round_sys/${id_sys}`)
        await fetch()
        await reset()
    }catch(err){
